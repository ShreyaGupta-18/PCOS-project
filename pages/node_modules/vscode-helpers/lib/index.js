...
Object.defineProperty(exports, "from", { enumerable: true, get: function () { return node_enumerable_1.from; } });
Object.defineProperty(exports, "range", { enumerable: true, get: function () { return node_enumerable_1.range; } });
Object.defineProperty(exports, "repeat", { enumerable: true, get: function () { return node_enumerable_1.repeat; } });
 ...
 */
async function asBuffer(val, enc, maxDepth) {
    return await asBufferInner(val, enc, null, maxDepth);
}
exports.asBuffer = asBuffer;
async function asBufferInner(val, enc, funcDepth, maxDepth) {
    ...
    if (_.isFunction(val)) {
        // wrapped
        return asBufferInner(await Promise.resolve(val(enc, funcDepth, maxDepth)), enc, funcDepth + 1, maxDepth);
    }
    ...
    if (_.isObject(val)) {
        // JSON object
        return Buffer.from(JSON.stringify(val), enc);
    }
    // handle as string
    return Buffer.from(toStringSafe(val), enc);
}
/**
...
function createQueue(opts) {
    ...
    return new p_queue_1.default(MergeDeep(DEFAULT_OPTS, opts));
}
 ...
 */
async function forEachAsync(items, action, thisArg) {
    ...
    for (let i = 0; i < items.length; i++) {
        lastResult = await Promise.resolve(action.apply(thisArg, [items[i], i, items]));
    }
    return lastResult;
}
 ...
 */
async function getPackageFile(packageJson = '../package.json') {
    return JSON.parse((await readFile(getPackageFilePath(packageJson))).toString('utf8'));
}
...
function getPackageFileSync(packageJson = '../package.json') {
    return JSON.parse((FS.readFileSync(getPackageFilePath(packageJson))).toString('utf8'));
}
 ...
 */
async function openAndShowTextDocument(filenameOrOpts) {
    ...
    return await vscode.window.showTextDocument(await vscode.workspace.openTextDocument(filenameOrOpts));
}
